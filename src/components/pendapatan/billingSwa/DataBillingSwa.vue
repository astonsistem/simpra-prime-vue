...
const openSyncDialog = async () => {
  ...
  try {
    // Dummy endpoint, ganti jika ada endpoint asli
    const response = await api.get('/syncapi/list/BILLING SWA')
...
const handleSyncRequest = async () => {
  ...
  try {
    // Ganti endpoint dan payload sesuai kebutuhan backend
    const payload = {
    ...
    // Contoh endpoint request preview data
    const response = await api.post(
      `/sinkronisasi/request-preview/swa/${selectedSync.value.value.id}`,
      payload
    )
    dataReq.value = response.data?.items || []
    toast.add({
      severity: 'success',
      summary: 'Berhasil',
    ...
    toast.add({ severity: 'error', summary: 'Gagal', detail: 'Gagal request data', life: 3000 })
  } finally {
...
const handleSyncSubmit = async () => {
  ...
  if (!dataReq.value.length) {
    toast.add({
      severity: 'warn',
      summary: 'Peringatan',
      detail: 'Request data terlebih dahulu',
      life: 3000,
    })
    return
  }
  ...
  try {
    ...
    await api.post(`/sinkronisasi/request/swa/${selectedSync.value.value.id}`, payload)
    toast.add({
      severity: 'success',
      summary: 'Berhasil',
...
const onConfirmDelete = async (event) => {
  ...
  const { id } = event.data
  try {
    await api.delete(`/billing_swa/${id}`)
    toast.add({
      severity: 'success',
      summary: 'Berhasil',
  ...
  <div class="p-4">
    ...
    >
      <div class="flex justify-between items-center mb-2">
        <h3 class="text-xl font-semibold text-[#17316E]">Data Billing 118</h3>
    ...
    >
      ...
      <div v-else class="space-y-4">
        ...
        <div v-if="dataReq.length" class="pt-4">
          <h4 class="font-semibold mb-2">Preview Data</h4>
          <DataTable :value="dataReq" scrollable scrollHeight="300px" class="p-datatable-sm">
            <Column
              v-for="(col, idx) in Object.keys(dataReq[0] || {})"
              :key="idx"
              :field="col"
              :header="col"
            />
    ...
    <ModalEditBillingSwa v-model="showModalEdit" :item="selectedItem" @saved="handleSaved" />
    <ModalValidasiBillingSwa
