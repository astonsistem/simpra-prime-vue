...
const loadData = async (page = 1, pageSize = rows.value) => {
  ...
  try {
    ...
    const response = await api.get('/penjamin', { params: query })
...
const initFilters = () => {
  filters.value = {
    ...
    penjamin_id: { value: null, matchMode: FilterMatchMode.CONTAINS },
    penjamin_nama: { value: null, matchMode: FilterMatchMode.CONTAINS },
    carabayar_id: { value: null, matchMode: FilterMatchMode.CONTAINS },
  }
}
initFilters()
...
const syncData = async () => {
  ...
  try {
    ...
    const res = await api.get('/penjamin/sync')
    ...
    if (res.data.count == 0) {
      ...
      toast.add({
        severity: 'success',
        summary: 'Berhasil',
        detail: `${res.data.count} Data Penjamin berhasil disinkronisasi`,
        life: 3000,
      })
    }
    ...
    toast.add({
      severity: 'error',
      summary: 'Gagal',
      detail: 'Gagal sinkronisasi Data Penjamin. Silakan coba lagi.',
      life: 3000,
    })
  } finally {
    loading.value = false
      ...
      >
        ...
        >
            ...
            <template #filter="{ filterModel }">
              <InputText v-model="filterModel.value" type="text" placeholder="Search Cara Bayar ID" />
            </template>
        </Column>
        <Column 
          field="penjamin_id" 
        ...
        >
            ...
            <template #filter="{ filterModel }">
              <InputText v-model="filterModel.value" type="text" placeholder="Search Penjamin ID" />
            </template>
        </Column>
        <Column 
          field="penjamin_nama" 
        ...
        >
            ...
            <template #filter="{ filterModel }">
              <InputText v-model="filterModel.value" type="text" placeholder="Search Penjamin Nama" />
            </template>
        </Column>
      </DataTable>
    </div>
