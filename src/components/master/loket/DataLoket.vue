...
const loadData = async (page = 1, pageSize = rows.value) => {
  ...
  try {
    ...
    const response = await api.get('/loket', { params: query })
...
const initFilters = () => {
  filters.value = {
    ...
    loket_id: { value: null, matchMode: FilterMatchMode.CONTAINS },
...
const syncData = async () => {
  ...
  try {
    ...
    const res = await api.get('/loket/sync')
    ...
    if (res.data.count == 0) {
      ...
      toast.add({
        severity: 'success',
        summary: 'Berhasil',
        detail: `${res.data.count} Data Loket berhasil disinkronisasi`,
        life: 3000,
      })
    }
    ...
    toast.add({
      severity: 'error',
      summary: 'Gagal',
      detail: 'Gagal sinkronisasi Data Loket. Silakan coba lagi.',
      life: 3000,
    })
  } finally {
    loading.value = false
  ...
  <div class="p-4">
    ...
    >
      <div class="flex justify-between items-center mb-2">
        <h3 class="text-xl font-semibold text-[#17316E]">Data Loket</h3>
        <div class="flex gap-2">
          ...
          <InputText v-model="filters['global'].value" placeholder="Search Loket..." />
        </div>
      </div>
      <DataTable
        :filters="filters"
      ...
      >
        ...
        >
            ...
            <template #filter="{ filterModel }">
              <InputText v-model="filterModel.value" type="text" placeholder="Search Loket ID" />
            </template>
        </Column>
        <Column 
          field="loket_nama" 
        ...
        >
            ...
            <template #filter="{ filterModel }">
              <InputText v-model="filterModel.value" type="text" placeholder="Search Loket Nama" />
            </template>
        </Column>
      </DataTable>
    </div>
