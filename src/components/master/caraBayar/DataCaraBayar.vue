...
const loadData = async (page = 1, pageSize = rows.value) => {
  ...
  try {
    ...
    const response = await api.get('/carabayar', { params: query })
...
const initFilters = () => {
  filters.value = {
    ...
    carabayar_nama: { value: null, matchMode: FilterMatchMode.CONTAINS },
  }
}
initFilters()
  ...
  <div class="p-4">
    ...
    >
      <div class="flex justify-between items-center mb-2">
        <h3 class="text-xl font-semibold text-[#17316E]">Data Cara Bayar</h3>
        <div class="flex gap-2">
          <Button label="Clear Column Filters" icon="pi pi-filter-slash" class="p-button-success" @click="clearTableFilters" />
          <InputText v-model="filters['global'].value" placeholder="Search Cara Bayar..." />
        </div>
      </div>
      <DataTable
        :filters="filters"
      ...
      >
        ...
        >
            ...
            <template #filter="{ filterModel }">
              <InputText v-model="filterModel.value" type="text" placeholder="Search Cara Bayar Nama" />
            </template>
        </Column>
      </DataTable>
    </div>
